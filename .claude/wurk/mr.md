# Opentelemetry_statifier extension: /wurk:mr
...
`CLAUDE.md` gained on 2026-09-01. Corrected on `ots-mp2`; the row always
governed.)
...
## The local gate is the verification; CI is a backstop

One contributor. CI runs the full gate, but only for pushes to `main` and
requests targeting `main`, and it runs after the push - so the full
`mix quality` before the push is still what verifies this branch. Treat a
skipped or scoped gate on a branch that touches gated paths as a hard stop,
not a formality CI will catch up on later. (A branch touching no gated paths
has no gate to run; say so in the request body.) What the request is for is
the record: a diff with a written rationale, linked to its bead, that can be
read later.

## Stacked requests: never rebase onto main

A branch cut from an unmerged sibling branch is a stacked request. The
generic skill's rebase-onto-the-default-branch step does **not** apply -
rebasing onto `main` flattens the stack into duplicated commits. Verify the
merge-base against the sibling branch instead, open the request with that
sibling as the base, as DRAFT while anything under it is unmerged, and name
the stack in the body so the merge order is on record. Note that a request
whose base is not `main` triggers no CI run.
