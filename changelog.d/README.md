...
- Span attributes for datamodel values are opt-in at setup instead of
  exported by default.
```
...
- Drops the `:include_datamodel` setup option. Datamodel values now attach
  only through an explicit attribute allowlist at setup.
```
...
- **Option removal**: The `:include_datamodel` option has been removed
  - **Rationale**: Unbounded values must never reach attributes by default
  - **Impact**: Callers must pass an allowlist
  - **Migration**: Replace the option with ...
```
